...
> - **Antivirus false positive** — Defender may flag the single-file `-portable.exe` as a threat (e.g. a `…!ml` heuristic detection like `Wacatac`/`Wacapew`). This is a **common false positive** for unsigned, self-extracting .NET single-file executables; treat the checksum and attestation above — not the antivirus label — as the source of truth for what you downloaded, and verify them before running. The build ships **uncompressed** to reduce the flag; if it still trips, use the **`…-win-x64.zip`** (loose-folder) download instead — the same app without the single-file packaging, which is not flagged.
