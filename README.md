...
- **`THIRD-PARTY-NOTICES.md`, `LICENSE.txt`, `lwext4-LICENSE.txt`, `DiskPartitionInfo-LICENSE.txt`** — licensing and third-party notices for the bundled native components. The disk-image library (`SharpExt4.dll`) statically embeds the **GPL-2.0** `lwext4`, so the release binary as an aggregate is conveyed under the GNU GPL v2; these files carry the required license texts and the corresponding-source offer. They also travel **inside** the loose-folder `.zip` beside the executable. See [`THIRD-PARTY-NOTICES.md`](THIRD-PARTY-NOTICES.md) and [`CLEANROOM.md`](CLEANROOM.md) for the full licensing position.
