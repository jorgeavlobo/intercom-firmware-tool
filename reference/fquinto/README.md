...
This repository's own code is **MIT-licensed** (`LICENSE.txt`), pending only the
copyright attribution — its `[year] [fullname]` placeholders are not yet filled
(tracked in issue #98) — and vendors **no GPL *source***. The upstream `main.py`
is **GPL-2.0** and is **not included here**, so this GPL script contributes no GPL
content to the source tree or the releases. If you fetch
