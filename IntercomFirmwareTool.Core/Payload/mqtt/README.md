...
| `ha_discovery.sh` | Publishes the retained Home Assistant MQTT discovery configs (from `ha/manifest` + `ha/*.json`, generated by the installer) once at startup when `HA_DISCOVERY=1`. Read-only; no command entity. |
