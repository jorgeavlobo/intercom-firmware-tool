...
`-stream_loop -1 -c copy` yields a continuous stream any consumer locks onto
within ~1 s. An **MP4 container is required** — a raw `.h264` does *not* loop
cleanly under `-stream_loop -c copy` (verified). Regenerate with:

```sh
# 1) render the annotated video to a raw Annex-B H.264 stream
ffmpeg -f lavfi -i "color=c=0x0b0e14:s=640x480:r=12:d=3" \
  -vf "drawtext=...:text='Loading camera'... animated dots ... drawtext=...:text='BTicino door camera'..." \
