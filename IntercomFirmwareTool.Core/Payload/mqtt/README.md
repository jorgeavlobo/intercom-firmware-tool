...
| `go2rtcd` | SysV init script for the **on-device camera media server** (issue #120), a byte-for-byte copy of the `btmqttd` script's supervision model retargeted at `/usr/sbin/go2rtc -config /etc/btmqttd/go2rtc/go2rtc.yaml`. Installed to `/etc/init.d/go2rtcd` (symlinked from `/etc/rc5.d/S99zGo2rtc`) **only on the on-device camera path** — a non-camera image never carries it. `start\|stop\|restart\|status\|respawn`. It also **opens the RTSP media port** (`tcp/8554` on `wlan0`, source-restricted to the LAN subnet, above the panel's `-P INPUT DROP`) when the daemon is **running** and closes it when stopped/disabled (phase 1c-3). It manages **exactly one INPUT rule** (interface + proto + dport + source subnet + ACCEPT) and nothing else — no sub-chain, no flush, no policy change. The rule carries a **required ownership tag** via the iptables comment match (`-m comment --comment go2rtcd`), and both insert and delete require that comment — so cleanup matches *only* rules bearing the tag, and a rule the panel or an admin may have added that is byte-for-byte identical **except** for the comment (even inserted above ours) is never matched and never removed (a bare rule spec is a matcher, not an identity). This is the same approach Docker/firewalld use to own their rules. The tag is **required**: if the kernel lacks the comment match (`xt_comment`) the script **fails closed** — it adds no rule and leaves `:8554` shut — rather than insert an untagged rule it couldn't distinguish from a foreign one and might delete on cleanup. `xt_comment` is present on essentially all Linux kernels, so this only ever affects a stripped kernel, and never at the cost of touching someone else's rule. The applied source subnet is remembered in a tmpfs file (`/var/run/go2rtc.fwrule`, cleared every boot with the ruleset) so a DHCP subnet change deletes the *old* rule precisely and `close` removes exactly what `open` added. It is idempotently **re-asserted every watchdog pass** (`respawn`) — the ruleset lives in tmpfs and the panel can rebuild its own chain, so a one-shot boot rule wouldn't survive — and it re-derives the LAN subnet each pass (DHCP-proof), skipping the open entirely until `wlan0` has an address so it never opens interface-wide. The go2rtc control API (`:1984`) stays loopback-only and gets no rule. |
