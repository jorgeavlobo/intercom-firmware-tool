...
| `presence.sh` | Holds a persistent MQTT session whose retained `offline` last will fires on an **unclean** drop, so HA availability (`TOPIC_LASTWILL`) tracks the bridge's real state. The orchestrator announces `online` when it (re)connects and `offline` explicitly on a clean shutdown. |
