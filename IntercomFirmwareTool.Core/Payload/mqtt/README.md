...
| `presence.sh` | Holds a persistent MQTT session whose retained `offline` last-will message is delivered on an **unclean** drop, so HA availability (`TOPIC_LASTWILL`) tracks the bridge's real state. It publishes the retained `online` **only after the broker's `SUBACK`** confirms the session connected (not on a timer), so `online` is never advertised without a live will-holder. The orchestrator publishes `offline` explicitly on a clean shutdown (which suppresses the will). |
