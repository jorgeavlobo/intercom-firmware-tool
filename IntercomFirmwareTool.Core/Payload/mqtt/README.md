...
| File | Role |
|---|---|
| `TcpDump2Mqtt` | Orchestrator: keeps sender/receiver/keypress/presence alive, publishes status, recovers from network outages. |
| `TcpDump2Mqtt.sh` | Init launcher (symlinked from `/etc/rc5.d/S99zTcpDump2Mqtt`). |
| `mqtt_common.sh` | Shared helper sourced by the scripts below: config load + `mqtt_pub`/`mqtt_sub_one`/`mqtt_presence` (auth+TLS) + the `remote_shell_allowed` gate. |
...
| `presence.sh` | Holds a persistent MQTT session whose retained `offline` last-will message the broker delivers on an **unclean** drop — the reliable half of HA availability (`TOPIC_LASTWILL`). The orchestrator refreshes the retained `online` each watchdog pass while presence is up (the session reconnects on its own, so a one-shot birth would stick at `offline` after a blip), and publishes `offline` explicitly on a clean shutdown (which suppresses the will). `online` is best-effort (see the availability limitation below); it errs toward a brief stale `offline` rather than a stale `online`. |
...
- **Availability `online` is best-effort.** `OFFLINE` is reliable (the broker
  delivers the presence session's retained last will on an unclean drop), but
  `ONLINE` cannot be made atomic with that connection using the mosquitto CLI
  tools: a separate `mosquitto_pub` can't share the subscriber's LWT connection,
  and the subscriber reconnects internally (so process liveness is not proof of a
  live session) while its `-d` output is block-buffered (so `SUBACK` can't be
  parsed for readiness). The orchestrator therefore refreshes retained `online`
  each watchdog pass while presence is up; it's coupled to the broker in practice
