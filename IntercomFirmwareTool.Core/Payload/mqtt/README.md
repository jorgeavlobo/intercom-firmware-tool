...
| `ha_discovery.sh` | Reconciles the retained Home Assistant MQTT discovery configs (from `ha/manifest` + `ha/*.json`, generated by the installer) once at startup: **publishes** them when `HA_DISCOVERY=1`, or **clears** them (empty retained payload) when `0`, so an opt-out removes the entities. Read-only; no command entity. |
