...
namespace IntercomFirmwareTool.Core
{
    ...
    public static class MqttInstaller
    {
        ...
        internal static string GenerateConf(MqttOptions opts)
        {
            ...
            // (universal), 0 = hi-res (C300X only).
            sb.Append("CAMERA_ENABLED=").Append(opts.CameraEnabled ? '1' : '0').Append('\n');
            // Serialize the RAW target, not EffectiveCameraTargetHost: the default (locked) case has
            // CameraTargetHost == null and MUST reach the device BLANK so config.rs falls back to
