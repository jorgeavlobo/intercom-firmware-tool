...
namespace IntercomFirmwareTool.Core
{
    ...
    public static class MqttInstaller
    {
        ...
        public static void InstallMqtt(ExtFileSystem fs, MqttOptions opts)
        {
            ...
            WriteConfigFile(fs, EtcDir + "/TcpDump2Mqtt.conf", GenerateConf(fs, opts), 600);
        ...
        public static IReadOnlyList<Ext4Check> ValidateMqtt(ExtFileSystem fs, MqttOptions opts)
        {
            ...
            var checks = new List<Ext4Check>();
            {
                ...
                // Config: 0600, and it names the host + primary topics.
                ...
                checks.Add(new(".conf sets MQTT_HOST", conf.Contains("MQTT_HOST="), ""));
                checks.Add(new(".conf sets TOPIC_RX", conf.Contains("TOPIC_RX="), ""));
                checks.Add(new(".conf sets TOPIC_DUMP", conf.Contains("TOPIC_DUMP="), ""));
                checks.Add(new(".conf ALLOW_REMOTE_SHELL matches option",
                    conf.Contains("ALLOW_REMOTE_SHELL=" + (opts.AllowRemoteShell ? "1" : "0")), ""));
                ...
                // Runtime deps the bridge needs (base tools; all confirmed present
                // on a factory C100X). jq/evtest are installed above, so not here.
                foreach (var dep in new[]
                {
                    "/usr/bin/mosquitto_pub", "/usr/bin/mosquitto_sub", "/usr/sbin/tcpdump",
                    "/usr/bin/python", "/etc/init.d/mosquitto",
                })
                    checks.Add(new($"runtime dep {dep} present",
                        fs.FileExists(dep) || fs.DirectoryExists(dep), ""));
            }
            return checks;
        }
        ...
        private static string GenerateConf(ExtFileSystem fs, MqttOptions opts)
        {
        ...
        private static void PatchFlexisip(ExtFileSystem fs)
        {
            ...
            if (content.Contains(marker)) return; // already patched
            ...
            // Back up the pristine file first (only if no backup exists yet).
