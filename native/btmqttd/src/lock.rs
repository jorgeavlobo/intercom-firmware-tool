//! Door-entry lock/actuator momentary-press pulse (issue #41), on its OWN tracked task.
//!
...
//!     release on the bus from rapid double-presses, even across the two actuators).
...
//!     press is never left without its release (which would hold the lock line
...
/// Depth of the lock request queue. Small: a lock is a low-frequency, human-driven
...
/// Gap between the lock press and release (ms) — long enough to register a real
/// momentary press, short enough to feel instant. Mirrors a physical button tap.
const LOCK_PULSE_MS: u64 = 300;
const LOCK_PULSE: Duration = Duration::from_millis(LOCK_PULSE_MS);

/// Per-frame forward timeout for the lock (ms) — TIGHTER than the raw-command path's
...
const LOCK_FORWARD_TIMEOUT_MS: u64 = 2_000;
const LOCK_FORWARD_TIMEOUT: Duration = Duration::from_millis(LOCK_FORWARD_TIMEOUT_MS);

/// The longest a single pulse can take: press + hold + release (two forwards each
/// bounded by [`LOCK_FORWARD_TIMEOUT`], plus the hold). DERIVED from the millis
...
pub const MAX_PULSE: Duration =
    Duration::from_millis(2 * LOCK_FORWARD_TIMEOUT_MS + LOCK_PULSE_MS);

/// Run the lock task: for each queued press request, emit the full momentary pulse
/// (press, hold, release) for the requested actuator, ONE AT A TIME. Returns when the
/// channel is closed and no pulse is in progress.
///
...
/// not-yet-started press has emitted nothing, so dropping it can't strand the lock.
...
/// One momentary pulse: press, hold [`LOCK_PULSE`], release. Each frame failure is
...
async fn pulse(lock: Lock) {
    ...
    if let Err(e) = forward_to_gateway(press, LOCK_FORWARD_TIMEOUT).await {
        eprintln!("btmqttd: {} press failed: {e}", lock.label());
    }
    tokio::time::sleep(LOCK_PULSE).await;
    if let Err(e) = forward_to_gateway(release, LOCK_FORWARD_TIMEOUT).await {
